...
function AddDebitCardModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
 ...
 <div
 ref={overlayRef}
 className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[2px] p-4"onClick={(e) => { if (e.target === overlayRef.current) onClose(); }}
 >
...
export default function BalancesPage() {
 ...
 payoutMsg.ok
 ? "border-[#BBF7D0] bg-[#F0FDF4] text-[#15803D]": "border-[#FECACA] bg-[#FEF2F2] text-[#991B1B]"}`}>
 ...
 instantAvailable
 ? "bg-[#DCFCE7] text-[#15803D]": "bg-[#F3F4F6] text-[#6B7280]"}`}>
 {instantAvailable ? (
 <>
 ...
 {payoutLoading
 ? "Procesando…": instantAvailable
 ? `Instant Payout · ${fmt(payoutNetCents, currency)}`: `Retirar · ${fmt(available, currency)}`}
 </button>
 {!loading && available === 0 && (
