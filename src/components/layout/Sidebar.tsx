  ...
  return (
    <div>
      <Link
        ...
        className="flex items-center gap-2 rounded-lg px-2.5 py-[7px] text-[12px] transition-all duration-100"
        style={active
      ...
      >
        ...
        {item.badge && !hasKids && (
          <span className="rounded-full px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide"
            style={{ background: theme.accentBg, color: theme.accentText }}>
            {item.badge}
          </span>
        )}
        {hasKids && (
      ...
      {hasKids && expanded && (
        <div className="ml-5 mt-0.5 space-y-0.5 pl-2.5"
...
export function Sidebar() {
  ...
  return (
    <aside
      className="flex h-screen w-[176px] shrink-0 flex-col"
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto px-2.5 pb-3 space-y-3 scrollbar-hide">
        {NAV_SECTIONS.map((section, si) => (
          <div key={si}>
            {section.title && (
              <p className="px-2.5 pb-1 pt-0.5 text-[10px] font-bold tracking-widest uppercase select-none"
                style={{ color: theme.sidebarMuted }}>
                {section.title}
              </p>
            )}
      ...
      <div className="px-2.5 pb-3 pt-1 border-t" style={{ borderColor: theme.sidebarBorder }}>
        <Link
          href="/app/settings"
          className="flex items-center gap-2 rounded-lg px-2.5 py-2 text-[11.5px] font-medium transition-colors hover:bg-black/5"
          style={{ color: theme.sidebarMuted }}
        >
