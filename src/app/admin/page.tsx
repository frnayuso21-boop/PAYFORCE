...
export default function AdminDashboard() {
 ...
 <StatCard
 icon={Users} label="Merchants"accent="text-slate-700"value={String(stats.merchants.total)}
 sub={`${stats.merchants.active} activos`}
 />
 <StatCard
 icon={TrendingUp} label="Ingresos totales"accent="text-emerald-600"value={fmt(stats.revenue.total)}
 sub={`${fmt(stats.revenue.month)} este mes`}
 />
 <StatCard
 icon={Euro} label="Comisiones plataforma"accent="text-blue-600"value={fmt(stats.fees.total)}
 sub={`${fmt(stats.fees.month)} este mes`}
 />
 <StatCard
 icon={CreditCard} label="Pagos procesados"accent="text-slate-700"value={String(stats.payments.total)}
 ...
 ? "bg-amber-50 border-amber-200": "bg-white border-slate-200"}`}>
 ...
 {stats.pendingPayouts > 0 && (
 <a href="/admin/payouts"className="ml-auto text-[12px] font-semibold text-amber-600 hover:underline">
 Gestionar →
 </a>
 )}
 </div>
 ...
 {stats.openDisputes > 0 && (
 <a href="/admin/disputes"className="ml-auto text-[12px] font-semibold text-red-600 hover:underline">
 Ver disputas →
 </a>
 )}
 </div>
