  ...
  async function sendPayment() {
    ...
    try {
      ...
      const waUrl       = fullPhone
        ? `https://wa.me/${fullPhone}?text=${msg}`
  ...
  if (step === "amount") return (
    <BottomSheet onClose={onClose} title="Cobrar por WhatsApp">
      <div className="flex flex-col items-center pt-1 pb-2">
        ...
        <input
          type="text"
          placeholder="Concepto (ej: cena del viernes)"
          value={description}
          onChange={(e) => setDescription(e.target.value)}
          className="w-64 rounded-xl bg-slate-100 px-4 py-2.5 text-center text-[14px] text-slate-700 placeholder:text-slate-400 outline-none"
          maxLength={60}
        />
      ...
      <button onClick={goToContact}
        ...
        className="w-full h-[54px] rounded-2xl bg-[#25D366] text-white text-[16px] font-bold active:scale-[0.98] disabled:opacity-30 transition-all flex items-center justify-center gap-2.5">
        <WhatsAppIcon />
    ...
    <BottomSheet onClose={onClose} title="¿A quién le cobras?">
      <div className="flex flex-col gap-4 pt-1 pb-2">
          ...
          <div className="flex items-center gap-2 rounded-2xl bg-slate-100 overflow-hidden">
            ...
            <div className="flex flex-1 items-center gap-2 pr-4">
              ...
              <input
                ...
                className="flex-1 bg-transparent text-[15px] text-slate-900 placeholder:text-slate-400 outline-none py-3.5"
                maxLength={15}
              />
            </div>
          </div>
        ...
        <button onClick={sendPayment} disabled={loading}
          className="w-full h-[54px] rounded-2xl bg-[#25D366] text-white text-[16px] font-bold active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2.5">
          {loading
  ...
  return (
    <BottomSheet onClose={onClose} title="">
      <div className="flex flex-col items-center gap-4 py-5 text-center">
        ...
        <div>
          <p className="text-[24px] font-bold text-slate-900">¡Solicitud enviada!</p>
          {customerName && (
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[90] rounded-t-3xl bg-white px-4 pt-4 shadow-2xl"
        ...
        <div className="flex items-center justify-between mb-4">
          {title ? <p className="text-[16px] font-bold text-slate-900">{title}</p> : <div />}
          <button onClick={onClose}
