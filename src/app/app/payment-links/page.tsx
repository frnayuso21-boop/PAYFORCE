...
function CreateModal({ onCreated, onClose }: { onCreated: () => void; onClose: () => void }) {
 ...
 <Button
 className="flex-1 gap-1.5 rounded-xl"onClick={() => copyToClipboard(result.url)}
 >
 <Copy className="h-4 w-4"/>
 Copiar URL
 </Button>
 <Button
 variant="outline"className="flex-1 gap-1.5 rounded-xl"onClick={() => window.open(result.url, "_blank")}
 >
 ...
 <button key={opt.key} type="button"onClick={() => setProductMode(opt.key as "none"| "select"| "new")}
 className="rounded-lg px-3 py-1.5 text-[12px] font-medium transition-all"style={productMode === opt.key
 ...
 <input type="text"placeholder="Nombre del producto"value={newProdName} onChange={(e) => setNewProdName(e.target.value)}
 ...
 <input type="number"min="0.50"step="0.01"placeholder="0,00"value={newProdPrice} onChange={(e) => setNewProdPrice(e.target.value)}
 className="flex-1 px-2 py-2 text-[13px] text-slate-800 outline-none placeholder:text-slate-300"/>
 </div>
 <input type="text"placeholder="Descripción (opcional)"value={newProdDesc} onChange={(e) => setNewProdDesc(e.target.value)}
 ...
 <input
 type="number"min="0.50"step="0.01"placeholder="0,00"value={form.amount}
 onChange={set("amount")}
 required
 ...
 <input
 type="text"placeholder="Ej: Factura #001 · Consultoría"value={form.description}
 ...
 <input
 type="text"placeholder="Ana García"value={form.customerName}
 ...
 <input
 type="email"placeholder="ana@email.com"value={form.customerEmail}
 ...
 <input
 type="tel"placeholder="612 345 678"value={form.customerPhone}
 ...
 <input
 type="checkbox"checked={form.reminderEnabled}
...
function RemindersTab() {
 ...
 title="Enviar recordatorio ahora"className="flex items-center gap-1 rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 py-1.5 text-[11px] font-medium text-indigo-600 hover:bg-indigo-100 disabled:opacity-50 transition-colors">
 ...
 title="Enviar por WhatsApp"className="flex items-center gap-1 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-1.5 text-[11px] font-medium text-emerald-600 hover:bg-emerald-100 transition-colors">
 ...
 title="Cancelar recordatorios"className="flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-[11px] font-medium text-slate-500 hover:bg-slate-100 disabled:opacity-50 transition-colors">
...
function MobileLinkCard({
 ...
 <span className="text-[18px] font-normal tabular-nums text-slate-900 leading-none">
 ...
 <a
 href={link.url}
 target="_blank"rel="noopener noreferrer"className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-slate-200 py-2 text-[12px] font-semibold text-slate-700 active:bg-slate-50">
...
export default function PaymentLinksPage() {
 ...
 {/* Overlay */}
 <div
 className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm"onClick={() => setModal(false)}
 />
 ...
 <div
 className="w-full rounded-t-3xl bg-white px-4 pt-3 pb-6 shadow-2xl md:max-w-md md:rounded-3xl md:p-6"style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 24px)"}}
 ...
 {/* Header */}
 <header
 className="sticky top-0 z-40 flex items-center justify-between border-b border-slate-100 bg-white px-4"style={{
 ...
 filter === f.value
 ? "bg-slate-900 text-white": "border border-slate-200 bg-white text-slate-500")}
 >
 {f.label}
 </button>
 ))}
 ...
 <a
 href="/app/connect/onboarding"className="mt-4 inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-[13px] font-semibold text-white">
