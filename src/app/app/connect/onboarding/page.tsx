...
function Stepper({ current }: { current: number }) {
 ...
 <div
 className="flex h-9 w-9 items-center justify-center rounded-full text-[12px] font-semibold transition-all"style={{
 ...
 <span
 className="text-[10px] font-medium"style={{ color: active ? "#0071e3": done ? "#1d1d1f": "#86868b"}}
 >
 {s.label}
 </span>
 </div>
 {i < STEPS.length - 1 && (
 <div
 className="w-12 h-px mx-1 mb-4 transition-colors"style={{ background: i < current ? "#1d1d1f": "#e5e5ea"}}
 />
 )}
 </div>
 );
...
export default function OnboardingPage() {
 ...
 <input className={inputCls} placeholder="Mi Tienda S.L. o Juan García"value={form.businessName} onChange={(e) => set("businessName", e.target.value)} />
 </Field>
 ...
 <input className={inputCls} placeholder="García López"value={form.lastName} onChange={(e) => set("lastName", e.target.value)} />
 </Field>
 </div>

 <Field label="Fecha de nacimiento"required>
 <div className="grid grid-cols-3 gap-2">
 <input className={inputCls} placeholder="DD"maxLength={2} inputMode="numeric"value={form.dobDay} onChange={(e) => set("dobDay", e.target.value)} />
 <input className={inputCls} placeholder="MM"maxLength={2} inputMode="numeric"value={form.dobMonth} onChange={(e) => set("dobMonth", e.target.value)} />
 <input className={inputCls} placeholder="AAAA"maxLength={4} inputMode="numeric"value={form.dobYear} onChange={(e) => set("dobYear", e.target.value)} />
 </div>
 </Field>

 <Field label="Dirección"required>
 <input className={inputCls} placeholder="Calle Mayor 1, 2º A"value={form.address} onChange={(e) => set("address", e.target.value)} />
 </Field>

 <div className="grid grid-cols-2 gap-3">
 <Field label="Ciudad"required>
 <input className={inputCls} placeholder="Madrid"value={form.city} onChange={(e) => set("city", e.target.value)} />
 </Field>
 <Field label="Código postal"required>
 <input className={inputCls} placeholder="28001"maxLength={5} inputMode="numeric"value={form.postalCode} onChange={(e) => set("postalCode", e.target.value)} />
 </Field>
 </div>

 <Field label="Teléfono"required>
 <input className={inputCls} placeholder="+34 600 000 000"type="tel"value={form.phone} onChange={(e) => set("phone", e.target.value)} />
 </Field>

 <Field label="DNI / NIE / Pasaporte"hint="Número de tu documento de identidad oficial.">
 <input className={inputCls} placeholder="12345678Z"value={form.idNumber} onChange={(e) => set("idNumber", e.target.value.toUpperCase())} />
 </Field>
 </>
 )}
 ...
 <input className={inputCls + "font-mono tracking-wider"} placeholder="ES91 2100 0418 4502 0005 1332"value={form.iban}
 onChange={(e) => {
 ...
 <Field label="Titular de la cuenta"hint="Nombre tal como aparece en tu cuenta bancaria.">
