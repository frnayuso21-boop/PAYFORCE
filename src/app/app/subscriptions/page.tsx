...
function TabCustomers() {
 ...
 amount: form.amount && priceType === "fixed"? Math.round(parseFloat(form.amount.replace(",", ".")) * 100)
 : undefined,
 }),
 });
 const d = await res.json();
 ...
 placeholder="MIEMPRESA·SUSCRIPCION"{...bind("bankDescriptor")}
 ...
 <button
 key={t} type="button"style={{ ...m.typeTab, ...(priceType === t ? m.typeTabActive : {}) }}
 ...
 <input
 type="text"inputMode="decimal"placeholder="0,00"{...bind("amount")}
 ...
 <button
 key={p.key} type="button"style={{ ...m.pill, ...(frequency === p.key ? m.pillActive : {}) }}
 ...
 <button
 type="submit"style={{ ...m.btnSubmit, opacity: creating ? 0.65 : 1 }}
 disabled={creating}
 >
...
function TabXML() {
 ...
 <SummaryCell label="Sin tarjeta "value={String(parseResult.withoutCard)} color="#B45309"/>
 ...
 : r.found
 ? { label: "Sin tarjeta", bg: "#FFFBEB", text: "#B45309"}
...
function TabHistory() {
 ...
 <thead>
 <tr>
 {["Archivo", "Fecha", "Total", "Cobrados", "Fallidos", "Sin tarjeta", "Estado", ""].map(h => (
 <th key={h} style={s.th}>{h}</th>
 ))}
 </tr>
 </thead>
...
function TabSettings() {
 ...
 <input
 defaultValue="Actualización sistema de cobros"style={s.input}
