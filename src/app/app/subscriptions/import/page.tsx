...
function DropZone({ onFile, loading }: { onFile: (f: File) => void; loading: boolean }) {
 ...
 ${drag
 ? "border-indigo-400 bg-indigo-50": "border-slate-200 bg-slate-50/60 hover:border-slate-300 hover:bg-slate-50"}
...
export default function SubscriptionsImportPage() {
 ...
 <Link
 href="/app/subscriptions"className="flex h-8 w-8 items-center justify-center rounded-full text-slate-400 hover:bg-slate-100 transition-colors">
 ...
 <FormatCard
 title="Excel / CSV"subtitle="Formato más común"cols={["Nombre", "Email", "Teléfono", "Importe", "Concepto", "MandatoId (opcional)"]}
 tip="Las columnas pueden estar en cualquier orden. Los nombres se detectan automáticamente en español e inglés."/>
 <FormatCard
 title="XML SEPA"subtitle="pain.008 y formatos similares"cols={["MndtId / EndToEndId", "Nm (Nombre deudor)", "InstdAmt (Importe)", "IBAN (opcional)", "EmailAdr (opcional)"]}
 ...
 <p className="mt-1 text-[22px] font-normal text-slate-900 leading-none">
 ...
 <StatChip icon=""value={result.withoutCard} label="Sin tarjeta"color="amber"/>
 ...
 <input
 type="checkbox"checked={sendInvites}
 ...
 <Link
 href="/app/subscriptions"className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-[13px] font-semibold text-white hover:bg-slate-700">
 Ver clientes
...
function ResultStat({ value, label, color }: {
 ...
 <p className={`text-[28px] font-normal ${colors[color]}`}>{value}</p>
