...
export const metadata: Metadata = { title: "Balance y Payouts — PayForce" };
...
export default async function PayoutsPage() {
  ...
  return (
    <>
      <MobileHeader title="Balance y Payouts" />
      <div className="w-full max-w-4xl mx-auto space-y-4 px-4 pt-3 pb-6 md:px-0 md:space-y-6 md:py-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-slate-900">Balance y retiradas</h1>
            <p className="mt-0.5 text-sm text-slate-500">
              Tu saldo disponible y el historial de transferencias a tu banco
            </p>
          </div>
        </div>

        <EmbeddedPayouts accountId={account.stripeAccountId} />
      </div>
    </>
  );
}
