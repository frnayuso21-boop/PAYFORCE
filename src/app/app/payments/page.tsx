...
function PaymentMethodBadge({ type, brand }: { type?: string | null; brand?: string | null }) {
 ...
 const label = brand && type === "card"? brand.charAt(0).toUpperCase() + brand.slice(1)
 : (PM_CONFIG[type]?.label ?? type);
 const cfg = PM_CONFIG[type] ?? { icon: <CreditCard className="h-3 w-3"/>, bg: "#F1F5F9", text: "#475569"};

 return (
 <span
 className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold"style={{ backgroundColor: cfg.bg, color: cfg.text }}
 >
 {cfg.icon}{label}
 </span>
 );
...
function StatusBadge({ status }: { status: string }) {
 ...
 return (
 <span
 className="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-semibold"style={{ backgroundColor: cfg.bg, color: cfg.text }}
 >
 {cfg.icon}{cfg.label}
 </span>
 );
...
function QuickRefundModal({
 ...
 className="flex items-center gap-1.5 rounded-xl px-4 py-2 text-[13px] font-medium text-white disabled:opacity-60 transition-colors"style={{ background: "#991B1B"}}>
...
export default function PaymentsPage() {
 ...
 className="flex items-center gap-1.5 text-[13px] text-slate-500 hover:text-slate-800 transition px-3 py-1.5 rounded-lg border border-slate-200"style={{ borderColor: "#E5E7EB"}}
 >
 ...
 className="rounded-md px-3 py-1.5 text-[12px] font-medium transition"style={{
 ...
 placeholder="Buscar por cliente, descripción, ID…"className="w-full rounded-lg border pl-9 pr-4 py-2 text-[13px] text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-200 transition"style={{ borderColor: "#E5E7EB"}}
 />
 </div>
 ...
 {/* Cabecera */}
 <div className="grid bg-slate-50 border-b text-[11px] font-semibold text-slate-400 uppercase tracking-wide px-5 py-3"style={{ borderColor: "#E5E7EB", gridTemplateColumns: "1.4fr 0.9fr 0.9fr 1.8fr 1.3fr 1.1fr 0.9fr 0.9fr 100px 24px"}}>
 <span>Importe</span>
 <span>Estado</span>
 <span>Método</span>
 ...
 <div
 key={i}
 className="grid items-center px-5 py-3.5"style={{ gridTemplateColumns: "1.4fr 0.9fr 0.9fr 1.8fr 1.3fr 1.1fr 0.9fr 0.9fr 100px 24px"}}
 >
 ...
 <div
 key={p.id}
 className="grid items-center px-5 py-3.5 hover:bg-slate-50 transition group"style={{ gridTemplateColumns: "1.4fr 0.9fr 0.9fr 1.8fr 1.3fr 1.1fr 0.9fr 0.9fr 100px 24px"}}
 >
 {/* Importe */}
 <Link href={`/app/payments/${p.id}`} className="contents">
 <span
 className="text-[14px] font-semibold tabular-nums"style={{
 color: p.status === "succeeded"? "#166534": p.status === "failed"? "#991B1B": "#475569",
 }}
 >
