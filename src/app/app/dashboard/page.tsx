...
function KpiCard({ label, value, sub, subPositive, loading: l }: {
 ...
 : <>
 <p className="text-[28px] font-normal tracking-[-1px] text-[#0A0A0A] leading-none">{value}</p>
 {sub && (
...
function DonutRing({ pct, label, sublabel }: { pct: number; label: string; sublabel: string }) {
 ...
 <circle cx="40"cy="40"r={r} fill="none"stroke="#0A0A0A"strokeWidth="8"strokeDasharray={`${dash} ${circ}`} strokeLinecap="round"transform="rotate(-90 40 40)"/>
 <text x="40"y="44"textAnchor="middle"fontSize="14"fontWeight="400"fill="#0A0A0A">
   {Math.round(pct)}%
  </text>
 </svg>
 <div className="text-center">
 <p className="text-[12px] font-normal text-[#0A0A0A]">{label}</p>
...
function QuickChargeModal({ onClose }: { onClose: () => void }) {
 ...
 { id: "qr", icon: "", label: "QR"},
 { id: "moto", icon: "", label: "Terminal MOTO"},
 ];
 ...
 return (
 <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4"onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
 ...
 className="flex items-center gap-2 rounded-xl border px-3 py-2.5 text-[12px] font-medium transition-all"style={method === m.id
 ...
 target="_blank"rel="noopener noreferrer"className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-[#25D366] py-2.5 text-[13px] font-bold text-white hover:opacity-90 transition">
 ...
 <input type="number"min="0.50"step="0.01"placeholder="0,00"value={amount} onChange={(e) => setAmount(e.target.value)}
 ...
 <input type="text"placeholder="Ej: Consultoría, factura #001…"value={concept} onChange={(e) => setConcept(e.target.value)}
...
export default function DashboardPage() {
 ...
 <p className="mt-2 text-[22px] font-normal tabular-nums text-slate-900 leading-none">{item.value}</p>
 </div>
 ))}
 </div>
 </MobileCard>
 ...
 <div className="grid grid-cols-2 gap-2">
 <a href={`https://wa.me/?text=${encodeURIComponent(`Aquí tienes tu enlace de pago: ${retryResult.url}`)}`} target="_blank"rel="noopener noreferrer"className="flex items-center justify-center gap-2 rounded-xl bg-[#25D366] py-2.5 text-[13px] font-bold text-white hover:opacity-90 transition">
 <MessageCircle className="h-4 w-4"/> WhatsApp
 </a>
 ...
 <div className="flex items-center gap-2">
 {lastUpdate && (
 <button onClick={() => fetchOverviewAndPayments()} title="Refrescar"className="flex items-center gap-1.5 rounded-[8px] px-2.5 py-1.5 text-[11px] text-[#9CA3AF] transition hover:bg-white hover:text-[#0A0A0A] border border-transparent hover:border-[#E5E7EB]">
 ...
 <Link href="/app/payment-links"className="flex items-center gap-1.5 rounded-[8px] bg-[#0A0A0A] px-3.5 py-1.5 text-[12px] font-medium text-white transition hover:bg-[#1a1a1a]">
 ...
 <KpiCard
 label="Cobrado hoy"value={formatCurrency(todayVol / 100)}
 sub={todayPmts.length > 0 ? `${todayPmts.length} pago${todayPmts.length > 1 ? "s": ""} hoy`: "Sin pagos hoy"}
 subPositive={todayPmts.length > 0}
 loading={loading}
 />
 <KpiCard
 label="Este mes"value={overview ? formatCurrency(overview.totalVolume / 100) : "0,00 €"}
 sub={overview?.comparison.volumeChange != null
 ? `${overview.comparison.volumeChange >= 0 ? "+": ""}${overview.comparison.volumeChange.toFixed(1)}% vs mes anterior`: undefined}
 subPositive={(overview?.comparison.volumeChange ?? 0) >= 0}
 loading={loading}
 />
 <KpiCard
 label="Ticket medio"value={formatCurrency(ticketMedio / 100)}
 sub={succeededPmts.length > 0 ? `${succeededPmts.length} pago${succeededPmts.length > 1 ? "s": ""} exitosos`: "Sin datos"}
 loading={loading}
 />
 <KpiCard
 label="Tasa de éxito"value={`${successRate.toFixed(1)}%`}
 ...
 <span className={cn("text-[10px]", isT ? "font-medium text-[#0A0A0A]": "text-[#9CA3AF]")}>{lbl}</span>
 </div>
 );
 })}
 </div>
 ...
 <>
 <p className="text-[14px] font-normal text-[#0A0A0A] mb-1 truncate">{topProduct[0]}</p>
 ...
 <div className="h-full rounded-full bg-[#0A0A0A] transition-all"style={{ width: `${totalPaymentSum > 0 ? Math.round((topProduct[1].sum / totalPaymentSum) * 100) : 0}%`}} />
 </div>
 ...
 {p.status === "SUCCEEDED"&& (
 <a href={`/api/invoices/${p.id}`} download title="Descargar factura PDF"className="inline-flex items-center gap-1 text-[10px] font-semibold text-indigo-600 hover:text-indigo-800 transition mt-0.5">
 ...
 title="Cobro rápido"className="hidden md:flex fixed bottom-6 right-6 z-[90] h-14 w-14 items-center justify-center rounded-full border-none text-white shadow-[0_4px_20px_rgba(0,0,0,0.18)] transition-transform hover:scale-105 active:scale-95"style={{ background: "#0A0A0A"}}>
