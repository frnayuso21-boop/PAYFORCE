...
function TerminalCard() {
 ...
 return (
 <div className="relative w-full overflow-hidden flex"style={{
 background: "#1a1a2e",
 ...
 {/* Barra */}
 <div className="flex items-center gap-3 px-4 py-3"style={{ borderBottom: "1px solid rgba(255,255,255,0.07)", background: "#181825"}}>
 ...
 <span className="shrink-0 select-none tabular-nums text-right"style={{ color: "rgba(255,255,255,0.18)", minWidth: 18, fontSize: 11 }}>
 {l.num}
 </span>
 ...
 {/* curl */}
 <div className="px-4 py-3 text-[11px]"style={{ borderTop: "1px solid rgba(255,255,255,0.06)", background: "#181825", fontFamily: "inherit"}}>
 ...
 <div className="flex flex-col w-[42%] shrink-0"style={{ background: "#16213e"}}>
 ...
 {/* Columnas */}
 <div className="flex justify-between px-4 py-1.5"style={{ borderBottom: "1px solid rgba(255,255,255,0.05)"}}>
 ...
 {events.map((e, i) => (
 <div key={i} className="flex items-center justify-between px-4 py-2"style={{ borderBottom: "1px solid rgba(255,255,255,0.04)"}}>
 ...
 <div className="h-6 w-6 rounded-full flex items-center justify-center text-[10px] font-bold"style={{ background: "rgba(189,147,249,0.25)", color: "#bd93f9"}}>P</div>
...
function DashboardOverview() {
 ...
 return (
 <div className="w-full overflow-hidden flex"style={{
 background: "#f8fafc",
 ...
 {/* Sidebar */}
 <div className="flex flex-col shrink-0 py-4 px-3 gap-1"style={{ width: 140, background: "#0f172a", borderRight: "1px solid rgba(255,255,255,0.06)"}}>

 {/* Logo */}
 <div className="flex items-center gap-2 px-2 mb-4">
 <div className="h-6 w-6 rounded-lg flex items-center justify-center text-white text-[10px] font-bold"style={{ background: "linear-gradient(135deg,#6366f1,#8b5cf6)"}}>P</div>
 <span className="text-[11px] font-semibold text-white/80">PayForce</span>
 </div>

 {navItems.map(n => (
 <div key={n.label} className="flex items-center gap-2 rounded-lg px-2 py-2 cursor-pointer"style={{
 ...
 {/* Badge TITAN */}
 <div className="mt-auto mx-1 rounded-lg p-2"style={{ background: "rgba(139,92,246,0.12)", border: "1px solid rgba(139,92,246,0.25)"}}>
 ...
 {/* Topbar */}
 <div className="flex items-center justify-between px-4 py-3"style={{ borderBottom: "1px solid #e8ecf1", background: "#ffffff"}}>
 <div>
 ...
 <span className="text-[9px] rounded-full px-2.5 py-1 text-white font-semibold"style={{ background: "linear-gradient(135deg,#6366f1,#8b5cf6)"}}>+ Cobro</span>
 </div>
 </div>
 ...
 {days.map((d, i) => (
 <text key={i} x={xs[i]} y={H - 1} textAnchor="middle"fontSize="7"fill={i === n-1 ? "#6366f1": "#94a3b8"} fontWeight={i === n-1 ? "700": "400"}
 ...
 <div className="flex items-center justify-between px-3 py-2"style={{ borderBottom: "1px solid #f8fafc"}}>
 <p className="text-[10px] font-semibold text-slate-800">Pagos recientes</p>
 <div className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse"/>
 </div>
 {payments.map((p, i) => (
 <div key={i} className="flex items-center gap-2 px-3 py-1.5"style={{ borderTop: i > 0 ? "1px solid #f8fafc": "none"}}>
 ...
 <span className="text-[8px] rounded-full px-1.5 py-0.5 shrink-0"style={{
...
function PaymentCheckoutVisual() {
 ...
 <div className="flex flex-col justify-between p-5 shrink-0"style={{ width: "40%", background: "#0f172a"}}>
 <div>
 {/* Logo */}
 <div className="flex items-center gap-2 mb-6">
 <div className="h-7 w-7 rounded-lg flex items-center justify-center font-bold text-white text-[11px]"style={{ background: "linear-gradient(135deg,#6366f1,#8b5cf6)"}}>P</div>
 ...
 {/* Importe */}
 <div className="rounded-xl px-4 py-4 mb-4"style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.10)"}}>
 ...
 <div key={l} className="flex justify-between text-[10px]"style={{ fontWeight: i === 2 ? 700 : 400, color: i === 2 ? "white": "rgba(255,255,255,0.40)"}}>
 ...
 {methods.map(m => (
 <button key={m.label}
 className="flex items-center justify-center gap-2 w-full rounded-xl py-2.5 text-[12px] font-semibold transition-all"style={{
 background: m.bg,
 color: m.color,
 ...
 {/* Card form */}
 <div className="flex flex-col gap-2">
 <div className="rounded-t-xl border px-3 py-2.5 flex items-center justify-between"style={{ borderColor: "#e2e8f0", borderBottom: "none"}}>
 ...
 <div className="flex rounded-b-xl border overflow-hidden"style={{ borderColor: "#e2e8f0", borderTop: "1px solid #f8fafc"}}>
 ...
 {cards.map(c => (
 <div key={c.name} className="rounded px-2 py-1 text-[8px] font-bold"style={{ background: c.color === "#EB001B"? "transparent": "#f8fafc", color: c.color,
 border: "1px solid #e8ecf1"}}>
 {c.name === "Mastercard"? <div className="flex -space-x-1"><div className="h-3 w-3 rounded-full bg-red-500 opacity-90"/><div className="h-3 w-3 rounded-full bg-orange-400 opacity-90"/></div>
 : c.name}
 </div>
 ))}
 ...
 {/* Botón pagar */}
 <button className="w-full rounded-xl py-3 text-[13px] font-bold text-white mt-auto"style={{ background: "linear-gradient(135deg,#6366f1,#8b5cf6)", boxShadow: "0 4px 16px rgba(99,102,241,0.35)"}}>
 Pagar €349,00
 </button>
 </div>
 </div>
...
function ShowcaseRow({ tag, title, body, checks = [], href, visual, cardShape = "all"}: ShowcaseRowProps) {
 ...
 {tag && (
 <span className="inline-block rounded-full px-3 py-1 text-[10px] tracking-[2px] uppercase md:text-[11px] md:tracking-[3px]"style={{ background: "rgba(124,58,237,0.15)", border: "1px solid rgba(124,58,237,0.3)", color: "#a78bfa"}}>
 {tag}
 </span>
 )}
 <h3 className="text-[36px] leading-tight text-white md:text-[56px]"style={{ letterSpacing: "-0.03em", fontWeight: 400 }}>
 {title.split("\n").map((line, i) => (
 ...
 <Link href={href}
 className="mt-1 inline-flex items-center gap-1.5 text-[13px] transition-opacity hover:opacity-60"style={{ color: "#a78bfa"}}>
 Saber más <ArrowRight className="h-3.5 w-3.5"/>
 </Link>
 </div>

 {/* Visual centrado full-width */}
 <div className="relative overflow-hidden w-full"style={{ borderRadius: radius, boxShadow: "0 40px 100px rgba(0,0,0,0.6)"}}>
 {visual}
 </div>
...
export function DashboardShowcase() {
 return (
 <section
 className="relative px-4 pt-16 pb-0 md:px-12 md:pt-24 overflow-hidden"style={{ background: "#000000"}}
 >
 {/* Capas decorativas */}
 ...
 <div className="pointer-events-none absolute inset-0"style={{ background: "radial-gradient(ellipse 55% 45% at 88% 8%, rgba(130,50,255,0.55) 0%, transparent 60%)"}} />

 {/* 4. Glow indigo — bottom-left */}
 <div className="pointer-events-none absolute inset-0"style={{ background: "radial-gradient(ellipse 50% 45% at 8% 92%, rgba(50,80,230,0.35) 0%, transparent 55%)"}} />

 {/* 5. Aurora central — barrido diagonal */}
 <div className="pointer-events-none absolute inset-0"style={{ background: "radial-gradient(ellipse 130% 40% at 50% 50%, rgba(100,40,200,0.14) 0%, transparent 70%)"}} />

 {/* 6. Teal accent — top-center (sutil) */}
 <div className="pointer-events-none absolute inset-0"style={{ background: "radial-gradient(ellipse 60% 30% at 50% 0%, rgba(13,210,185,0.09) 0%, transparent 65%)"}} />

 {/* Difuminado superior — funde con el hero */}
 <div className="pointer-events-none absolute top-0 left-0 right-0 z-10"style={{ height: 180, background: "linear-gradient(to bottom, #04020e 0%, transparent 100%)"}} />

 <div className="mx-auto max-w-5xl flex flex-col gap-24 md:gap-36 relative z-0 pt-28 pb-24 md:pb-36">

 {/* Fila 1 — SDK Terminal */}
 <ShowcaseRow
 tag="API & SDK"title={"Conecta en\nminutos, no días"}
 body="Una sola API unifica todos los métodos de pago: tarjeta, Bizum, Apple Pay, SEPA y más. Sin integraciones separadas, sin complicaciones."checks={[
 "SDK oficial en TypeScript, Python y PHP",
 "Webhooks en tiempo real con firma HMAC",
 "Documentación interactiva con ejemplos",
 ]}
 href="/solutions"visual={<TerminalAnimated />}
 cardShape="all"/>

 {/* Fila 2 — Dashboard analytics */}
 <ShowcaseRow
 tag="Analytics"title={"Dashboard en\ntiempo real"}
 body="Monitoriza cada transacción, detecta anomalías y consulta métricas de rendimiento desde un panel centralizado con datos en vivo."checks={[
 "Métricas de conversión y volumen por hora",
 "Alertas inteligentes ante fraude o caídas",
 "Exportación a CSV, PDF y Zapier",
 ]}
 href="/solutions"visual={<DashboardOverviewClient />}
 cardShape="all"/>

 {/* Fila 3 — Checkout */}
 <ShowcaseRow
 tag="LINK DE PAGO"title={"Checkout listo\nen segundos"}
 body="Tus clientes pagan con Apple Pay, Google Pay, Bizum, tarjeta o SEPA sin ninguna integración. Comparte un link y cobra al instante."checks={[
 "Apple Pay, Google Pay y Bizum nativos",
 "Tarjeta, SEPA y transferencia en un solo link",
 "Sin código: actívalo desde el dashboard",
 ]}
 href="/solutions"visual={<PaymentCheckoutVisual />}
 cardShape="curveTop"/>
