...
const inputCls = (err?: string) =>
 ...
 err
 ? "border-red-300 focus:border-red-400 focus:ring-red-100": "border-slate-200 focus:border-slate-400 focus:ring-slate-100"}`;
...
function Section({ icon, title, children, collapsible = false, defaultOpen = true }: {
 ...
 <button
 type="button"onClick={() => collapsible && setOpen(o => !o)}
...
export function CreateCustomerModal({ open, onClose, onSaved }: Props) {
 ...
 return (
 <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4"style={{ background: "rgba(15,23,42,0.55)", backdropFilter: "blur(4px)"}}
 onClick={e => e.target === e.currentTarget && handleClose()}
 >
 <div className="w-full sm:max-w-2xl bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden"style={{ maxHeight: "96dvh"}}
 >
 {/* Header */}
 ...
 {/* Cuerpo con scroll */}
 <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 py-5 space-y-4"style={{ scrollbarWidth: "thin"}}>
 ...
 <input className={inputCls(errors.name)} placeholder="María García"value={form.name} onChange={e => set("name", e.target.value)} />
 </Field>
 <Field label="Correo electrónico"required error={errors.email}>
 <input type="email"className={inputCls(errors.email)} placeholder="maria@empresa.com"value={form.email} onChange={e => set("email", e.target.value)} />
 </Field>
 </div>
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
 <Field label="Idioma">
 ...
 <input className={inputCls()} placeholder="Cliente premium, empresa de software…"value={form.description} onChange={e => set("description", e.target.value)} />
 </Field>
 </div>
 ...
 <input className={inputCls()} placeholder="Acme Corp S.L."value={form.companyName} onChange={e => set("companyName", e.target.value)} />
 </Field>
 <Field label="Contacto principal">
 <input className={inputCls()} placeholder="Pedro Sánchez"value={form.contactName} onChange={e => set("contactName", e.target.value)} />
 </Field>
 </div>
 </div>
 </Section>
 ...
 placeholder="facturacion@empresa.com"disabled={form.sameBillingEmail}
 ...
 <input className={inputCls()} placeholder="612 345 678"value={form.phone} onChange={e => set("phone", e.target.value)} />
 </div>
 </Field>
 </div>
 ...
 <input className={inputCls()} placeholder="B-12345678"value={form.taxId} onChange={e => set("taxId", e.target.value)} />
 </Field>
 </div>
 </Section>
 ...
 <input className={inputCls()} placeholder="María García"value={form.shippingName} onChange={e => set("shippingName", e.target.value)} />
 </Field>
 <Field label="Dirección">
 <input className={inputCls()} placeholder="Calle Mayor 1, 2ºA"value={form.shippingAddress} onChange={e => set("shippingAddress", e.target.value)} />
 </Field>
 <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
 <Field label="Ciudad">
 <input className={inputCls()} placeholder="Madrid"value={form.shippingCity} onChange={e => set("shippingCity", e.target.value)} />
 </Field>
 <Field label="C. Postal">
 <input className={inputCls()} placeholder="28001"value={form.shippingPostalCode} onChange={e => set("shippingPostalCode", e.target.value)} />
 </Field>
 <Field label="Provincia">
 <input className={inputCls()} placeholder="Madrid"value={form.shippingState} onChange={e => set("shippingState", e.target.value)} />
 </Field>
 <Field label="País">
 <div className="relative">
 ...
 <button
 key={opt.value}
 type="button"onClick={() => set("invoiceTemplate", opt.value)}
 ...
 ? "border-slate-900 bg-slate-50": "border-slate-200 hover:border-slate-300 bg-white"}`}
 >
 ...
 className="rounded-xl px-5 py-2.5 text-[13px] font-semibold text-white transition disabled:opacity-60 flex items-center gap-2"style={{ background: saved ? "#16a34a": "#0f172a"}}
 >
 {saved ? (
