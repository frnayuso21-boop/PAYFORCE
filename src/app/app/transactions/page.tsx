...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-[980px] px-2 py-0.5 text-[10px] font-medium"
...
function SkRow() {
  return (
    <div className="grid items-center px-5 py-3.5 border-b border-[#F3F4F6]"
      style={{ gridTemplateColumns: "1.4fr 1fr 1.8fr 1.6fr 1.2fr 1fr 1fr 90px 20px" }}>
  ...
  return (
    <div className="min-h-full bg-[#F9FAFB] p-6 lg:p-8">
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <div className="flex items-center gap-1 rounded-[8px] border border-[#E5E7EB] bg-white p-1">
          {FILTERS.map((f) => (
            <button key={f.key} onClick={() => setFilter(f.key)}
              className="rounded-[6px] px-3 py-1.5 text-[11px] font-medium transition"
              style={{ background: filter === f.key ? "#0A0A0A" : "transparent", color: filter === f.key ? "#fff" : "#6B7280" }}>
              {f.label}
            </button>
          ))}
        </div>
        <div className="relative flex-1 max-w-sm">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)}
            ...
            className="w-full rounded-[8px] border border-[#E5E7EB] bg-white pl-9 pr-4 py-2 text-[12px] text-[#0A0A0A] placeholder-[#9CA3AF] focus:outline-none focus:ring-1 focus:ring-[#0A0A0A] transition" />
        </div>
        {visible.length > 0 && (
      ...
      <div className="rounded-[10px] border border-[#E5E7EB] bg-white overflow-hidden">
        {/* Cabecera */}
        <div className="grid border-b border-[#E5E7EB] px-5 py-2.5 text-[10px] font-medium uppercase tracking-[0.06em] text-[#9CA3AF]"
          style={{ gridTemplateColumns: "1.4fr 1fr 1.8fr 1.6fr 1.2fr 1fr 1fr 90px 20px" }}>
          <span>Importe</span>
          <span>Estado</span>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-[#F3F4F6]">
            {visible.map((p) => (
              <div key={p.id}
                className="grid items-center px-5 py-3 hover:bg-[#F9FAFB] transition group"
                style={{ gridTemplateColumns: "1.4fr 1fr 1.8fr 1.6fr 1.2fr 1fr 1fr 90px 20px" }}>

                {/* Importe + método */}
                <Link href={`/app/payments/${p.id}`} className="block min-w-0 pr-2">
                  <p className="text-[13px] font-semibold tabular-nums"
                    style={{ color: p.status === "succeeded" ? "#0A0A0A" : p.status === "failed" ? "#991B1B" : "#6B7280" }}>
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  {p.status === "succeeded" && (
                    <a href={`/api/invoices/${p.id}`} download title="Factura PDF"
                      className="text-[#9CA3AF] hover:text-[#0A0A0A] transition">
